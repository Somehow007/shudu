...
import { initDesktopStorage } from './platform/storage';
...
function UpdateBanner({ onUpdate, onDismiss }: { onUpdate: () => void; onDismiss: () => void }) {
  return (
    <div className="desktop-update-banner">
      <span>🔄 发现新版本</span>
      <button className="desktop-update-banner__btn" onClick={onUpdate}>
        立即更新
      </button>
      <button className="desktop-update-banner__dismiss" onClick={onDismiss}>✕</button>
    </div>
  );
}
